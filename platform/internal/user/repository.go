...
const userColumns = `id, email, COALESCE(password_hash, ''), role, display_name, timezone, locale, display_currency,
...
func scanUser(row pgx.Row) (*model.User, error) {
	u := &model.User{}
	err := row.Scan(&u.ID, &u.Email, &u.PasswordHash, &u.Role, &u.DisplayName, &u.Timezone, &u.Locale,
		&u.DisplayCurrency, &u.AdvertiserID,
		&u.PublisherID,
...
func (r *Repository) Update(ctx context.Context, u *model.User) error {
	_, err := r.pool.Exec(ctx, `
		UPDATE platform_users SET display_name = $2, timezone = $3, locale = $4, display_currency = $5,
			updated_at = NOW() WHERE id = $1`,
		u.ID, u.DisplayName, u.Timezone, u.Locale, u.DisplayCurrency,
	)
	return err
}
